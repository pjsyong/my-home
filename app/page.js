              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-5 py-2 rounded-full whitespace-nowrap text-sm font-bold transition-all ${
                    selectedCategory === cat ? "bg-orange-500 text-white shadow-md shadow-orange-200" : "bg-white text-slate-500 border border-slate-200"
                  }`}
                >
                  {cat}
                </button>
        ...
        <div className="max-w-xl mx-auto p-4 space-y-4 mt-2">
          {loading ? (
            ...
            <div key={item.id} className="relative bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
              ...
              <h2 className="text-2xl font-black text-slate-800 mb-4 pr-12">{item.title}</h2>
              <div className="bg-slate-50 p-4 rounded-2xl text-slate-600 text-[15px] leading-relaxed whitespace-pre-wrap font-medium">
                {item.component}
              </div>
              {item.feedback && (
                <div className="mt-4 flex gap-2 items-start bg-blue-50/50 p-3 rounded-xl border border-blue-100">
        ...
        {editingItem && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-xl rounded-t-[2.5rem] sm:rounded-[2.5rem] p-8 shadow-2xl animate-in slide-in-from-bottom duration-300">
              ...
              <form onSubmit={requestUpdate} className="space-y-4 overflow-y-auto max-h-[70vh] pr-2 no-scrollbar">
                <div>
                  <label className="text-xs font-bold text-slate-400 ml-1">카테고리</label>
                  <input 
                    className="w-full p-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-orange-500 font-bold text-slate-900"
                ...
                <div>
                  ...
                  <input 
                    className="w-full p-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-orange-500 font-black text-lg text-slate-900"
                ...
                <div>
                  ...
                  <textarea 
                    rows={6}
                    className="w-full p-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-orange-500 font-medium text-slate-900"
                    value={editingItem.component || ""}
                    onChange={(e) => setEditingItem({...editingItem, component: e.target.value})}
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-blue-500 ml-1">조리 팁 (선택)</label>
                  <textarea 
                    rows={2}
                    className="w-full p-4 bg-blue-50/50 rounded-2xl border border-blue-100 focus:ring-2 focus:ring-blue-500 font-medium text-blue-600"
                    value={editingItem.feedback || ""}
                    onChange={(e) => setEditingItem({...editingItem, feedback: e.target.value})}
                  />
                </div>
                <button type="submit" className="w-full p-5 bg-orange-500 text-white rounded-3xl font-black text-lg shadow-lg shadow-orange-200 active:scale-95 transition-transform mt-4">
                  수정 완료 저장
                </button>
              </form>
            </div>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-md">
            <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl text-center animate-in zoom-in duration-200">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button onClick={handleUpdateFinal} className="p-4 bg-orange-500 text-white rounded-2xl font-black shadow-lg shadow-orange-200">확인</button>
              </div>
            </div>
          </div>
        )}
